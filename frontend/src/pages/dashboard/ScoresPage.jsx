...
function ScoreForm({ onSubmit, defaultValues, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div>
        <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Score (1–45)</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Date</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Notes (optional)</label>
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '0.9rem' }}>{scores.length}/{MAX} scores stored • Newest first</p>
        </div>
      ...
      <div style={{ padding: '0.875rem 1rem', background: 'rgba(99,102,241,0.06)', borderRadius: '12px', border: '1px solid rgba(99,102,241,0.15)', marginBottom: '1.5rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
        ...
        <p style={{ fontSize: '0.8rem', color: '#94a3b8', lineHeight: 1.6 }}>
          Only your <strong style={{ color: '#818cf8' }}>5 most recent scores</strong> are stored. Score range: <strong style={{ color: '#818cf8' }}>1–45</strong>. Each score must have a unique date. Adding a 6th score replaces the oldest.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          style={{ textAlign: 'center', padding: '4rem 2rem', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>⛳</div>
          <h3 style={{ fontWeight: 700, marginBottom: '0.5rem' }}>No scores yet</h3>
          <p style={{ color: '#64748b', marginBottom: '1.5rem' }}>Add your first golf score to start participating in draws</p>
          ...
          <AnimatePresence>
            {scores.map((score, i) => (
              <motion.div key={score.id} variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }}
                ...
                style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                {/* Score number */}
                <div style={{ width: '56px', height: '56px', borderRadius: '14px', background: 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(139,92,246,0.2))', border: '1px solid rgba(99,102,241,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '1.25rem', color: '#818cf8', fontFamily: 'Outfit', flexShrink: 0 }}>
                  {score.score}
                </div>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#64748b' }}>{new Date(score.score_date).toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}</span>
                  </div>
                  {score.notes && <p style={{ fontSize: '0.85rem', color: '#94a3b8' }}>{score.notes}</p>}
                </div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => setEditScore(score)} style={{ width: '36px', height: '36px', borderRadius: '8px', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', cursor: 'pointer', color: '#818cf8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Edit2 size={15} />
                  </button>
      ...
      <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete Score">
        {deleteConfirm && (
          <div>
            <p style={{ color: '#94a3b8', marginBottom: '1.5rem' }}>Are you sure you want to delete the score <strong style={{ color: '#f8fafc' }}>{deleteConfirm.score}</strong> from {new Date(deleteConfirm.score_date).toLocaleDateString()}?</p>
