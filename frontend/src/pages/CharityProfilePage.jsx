...
export default function CharityProfilePage() {
  ...
  if (!charity) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: '80px', color: 'var(--text-muted)' }}>Charity not found.</div>;
    ...
    <div style={{ paddingTop: '80px', minHeight: '100vh' }}>
      ...
      <div style={{ height: '280px', background: 'linear-gradient(135deg, rgba(16,185,129,0.2) 0%, rgba(99,102,241,0.15) 100%)', display: 'flex', alignItems: 'flex-end', padding: '2rem 1.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <Link to="/charities" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem', color: 'var(--text-secondary)', fontSize: '0.875rem', textDecoration: 'none', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', flexWrap: 'wrap' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', background: 'var(--bg-border-light)', padding: '0.2rem 0.625rem', borderRadius: '6px' }}>{charity.category}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem', alignItems: 'start' }}>
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--bg-border-light)', padding: '1.75rem', marginBottom: '1.5rem' }}>
            ...
            {charity.charity_events?.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--bg-border-light)', padding: '1.75rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {charity.charity_events.map((event) => (
                    <div key={event.id} style={{ padding: '1rem', background: 'var(--bg-border-light)', borderRadius: '10px', border: '1px solid var(--bg-border-light)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.375rem' }}>
                        <Calendar size={14} color="#818cf8" />
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{event.event_date ? new Date(event.event_date).toLocaleDateString() : 'Ongoing'}</span>
                      </div>
                      <h4 style={{ fontWeight: 600, marginBottom: '0.25rem', fontSize: '0.9rem' }}>{event.title}</h4>
                      {event.description && <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{event.description}</p>}
                    </div>
                  ))}
                </div>
              </motion.div>
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--bg-border-light)', padding: '1.5rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Total raised via Golf Impact</div>
                </div>
                <div>
                  ...
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Golf Impact donors</div>
                </div>
              </div>
            </motion.div>
