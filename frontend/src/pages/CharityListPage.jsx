    ...
    <div style={{ paddingTop: '80px', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '3rem 1.5rem 2rem', background: 'radial-gradient(ellipse at top, rgba(16,185,129,0.08) 0%, transparent 60%)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} style={{ color: 'var(--text-muted)', fontSize: '1.1rem', marginBottom: '2rem' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem 1.5rem' }}>
          ...
          {['', ...categories].map((cat) => (
            <button key={cat || 'all'} onClick={() => setActiveCategory(cat)}
              style={{ padding: '0.375rem 1rem', borderRadius: '9999px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s',
                background: activeCategory === cat ? 'rgba(16,185,129,0.15)' : 'var(--bg-border-light)',
                border: activeCategory === cat ? '1px solid rgba(16,185,129,0.4)' : '1px solid var(--bg-border-light)',
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>No charities found. Try adjusting your search.</div>
        ) : (
            ...
            {charities.map((charity, i) => (
              <motion.div key={charity.id} variants={fadeUp} initial="hidden" animate="visible" transition={{ delay: i * 0.05 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', background: 'var(--bg-border-light)', padding: '0.2rem 0.5rem', borderRadius: '6px' }}>{charity.category}</span>
                ...
                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: '1.25rem' }}>{charity.short_description}</p>
                <Link to={`/charities/${charity.id}`} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-primary-light)', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none' }}>
