    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>{pagination.total || 0} total verifications</p>
        </div>
          ...
          {STATUS_FILTERS.map((s) => (
            <button key={s || 'all'} onClick={() => { setStatusFilter(s); setPage(1); }}
              style={{ padding: '0.375rem 0.875rem', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s',
                ...
                border: statusFilter === s ? `1px solid ${statusColors[s] || '#6366f1'}40` : '1px solid var(--bg-border-light)',
                color: statusFilter === s ? (statusColors[s] || 'var(--color-primary-light)') : 'var(--text-muted)',
              }}>
      ...
      {loading ? <div style={{ display: 'flex', justifyContent: 'center', paddingTop: '3rem' }}><Spinner size={32} /></div> : (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--bg-border-light)', borderRadius: '16px', overflow: 'auto' }}>
          <table className="data-table">
            ...
            <tbody>
              {verifications.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>No verifications found</td></tr>
              ) : verifications.map((v) => (
                <tr key={v.id}>
                  <td>
                    <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.875rem' }}>{v.users?.full_name}</div>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{v.users?.email}</div>
                  </td>
                  ...
                  <td>
                    {v.proof_url ? (
                      <a href={v.proof_url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary-light)', fontSize: '0.8rem' }}>View Proof</a>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.375rem' }}>
                      {v.status === 'pending' && (
                        <button onClick={() => setReviewModal(v)} style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '8px', padding: '0.3rem 0.6rem', cursor: 'pointer', color: 'var(--color-primary-light)', fontSize: '0.75rem', fontWeight: 600 }}>
                          Review
                        </button>
                      )}
                      {v.status === 'approved' && (
      ...
      <Modal isOpen={!!reviewModal} onClose={() => { setReviewModal(null); setNotes(''); }} title="Review Winner Verification">
        {reviewModal && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ padding: '1rem', background: 'var(--bg-border-light)', borderRadius: '10px' }}>
              <p><strong style={{ color: 'var(--text-primary)' }}>{reviewModal.users?.full_name}</strong></p>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Prize: ${parseFloat(reviewModal.prize_amount).toFixed(2)}</p>
              {reviewModal.proof_url && <a href={reviewModal.proof_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.85rem', color: 'var(--color-primary-light)' }}>View Proof Document →</a>}
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Admin Notes (optional)</label>
